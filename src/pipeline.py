...
from modeling.create_redshift_schema import create_redshift_schema
...
from load.redshift_copy_loader import vacuum_analyze
...
if __name__ == "__main__":
    ...
    if os.getenv("DB_ENGINE") == "redshift":
    ...
    if os.getenv("DB_ENGINE") == "redshift":
        conn = db_connect()
        vacuum_analyze(conn, "stock_prices")
        vacuum_analyze(conn, "dim_date")
        vacuum_analyze(conn, "dim_metadata")
        conn.close()